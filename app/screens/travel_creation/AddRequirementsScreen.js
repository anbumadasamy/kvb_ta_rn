...
export default function AddRequirementsScreen({ route }) {
  const navigation = useNavigation();
  const authCtx = useContext(AuthContext);
  const reqFroms = route.params.reqFrom;
  ...
  useEffect(() => {
    ...
    if (
      ...
      inputChangedHandler("roomType", route.params.requirementsDetail.roomType);
      inputChangedHandler(
        "roomTypeId",
      ...
      inputChangedHandler("fromDate", route.params.requirementsDetail.fromDate);
      inputChangedHandler("fromTime", route.params.requirementsDetail.fromTime);
      inputChangedHandler(
        "fromPlace",
  ...
  const handleConfirmStartDate = (selectedDate) => {
    ...
    inputChangedHandler("fromDateJson", selectedDate);
  };
  ...
  const handleConfirmCheckInDate = (selectedDate) => {
    ...
    inputChangedHandler("checkInDateJson", selectedDate);
  };
  ...
  const handleConfirmCheckOutDate = (selectedDate) => {
    ...
    inputChangedHandler("checkOutDateJson", selectedDate);
  };
  ...
  async function getBookingNeeded() {
    try {
      ...
      console.log("Booking needed :>> " + JSON.stringify(json));
  ...
  function AddRequirements() {
    if (requirementsData.bookingNeeded && requirementsData.bookingNeededId) {
      if (requirementsData.bookingNeededId == 1) {
        if (requirementsData.roomType && requirementsData.roomTypeId) {
        ...
        if (requirementsData.cabSegmentId && requirementsData.cabSegment) {
          if (requirementsData.travelTypeCabId && requirementsData.cabType) {
